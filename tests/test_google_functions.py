...
def test_get_photo_of_place(mocker):
    ...
    assert google_functions.get_photo_of_place(
        "Eiffel Tower", "fake_api_key") == f'{constants.PLACE_IMAGE_PATH}/{constants.RHODES_IMAGE}'
    ...
    mocked_response_places.json.return_value = {
        "candidates": [{
            "photos": [{
                "photo_reference": "fake_photo_reference"
            }]
        }]
    }
    ...
    mocked_get.side_effect = [
        mocked_response_places, mocked_response_photo]

    # Test a successful photo fetch and save
    assert google_functions.get_photo_of_place(
        "Eiffel Tower", "fake_api_key") is not None
    ...
    assert google_functions.get_photo_of_place(
        "Eiffel Tower", "fake_api_key") is None
