...
@pytest.mark.parametrize(
    ...
    [
        ("London, England. I suggest visiting London, England for your holiday.", {"Place": "London", "Country": "England"}),
        ("New York, USA. I suggest visiting New York, USA for your holiday.", {"Place": "New York", "Country": "USA"}),
...
def test_fetch_and_extract_destination_testing_false(mocker):
    ...
    mocker.patch("src.chatgpt_functions.extract_place_country", return_value={"Place": "Rhodes", "Country": "Greece"})

    fake_response = {
        "choices": [
            {"message": {"content": "Suggested content"}}
        ]
    }
...
@pytest.mark.parametrize(
    ...
    [
        ("London", "UK", None, None),
        ("New York", "USA", type("Location", (), {"latitude": 40.7128, "longitude": -74.0060})(), {"Latitude": 40.7128, "Longitude": -74.0060}),
    ],
)
def test_get_lat_lon(mocker, input_place, input_country, mock_location, expected_output):
    mock_geolocator = mocker.patch.object(Nominatim, "geocode", return_value=mock_location)
