...
def test_haversine_distance():
    ...
    assert pytest.approx(flight_functions.haversine_distance(
        52.5200, 13.4050, 48.8566, 2.3522), 878)  # Berlin to Paris
    assert pytest.approx(flight_functions.haversine_distance(
        34.0522, -118.2437, 36.7783, -119.4179), 329)  # LA to Fresno
    assert pytest.approx(flight_functions.haversine_distance(
        40.7128, -74.0060, 34.0522, -118.2437), 3940)  # NYC to LA
    assert pytest.approx(flight_functions.haversine_distance(
        37.7749, -122.4194, 34.0522, -118.2437), 559)  # SF to LA
    
    # Test zero distance (same point)
    assert flight_functions.haversine_distance(
        52.5200, 13.4050, 52.5200, 13.4050) == 0.0
    
    # Test negative coordinates
    assert pytest.approx(flight_functions.haversine_distance(
        -34.6037, -58.3816, -33.8688, 151.2093), 11520)  # Buenos Aires to Sydney
    
    # Test large distance (almost antipodal points)
    assert pytest.approx(flight_functions.haversine_distance(
        37.7749, -122.4194, -37.8136, 144.9631), 12658)  # SF to Melbourne
...
def test_nearest_location():
    ...
    # Test from Berlin
    assert flight_functions.nearest_location(
        52.5200, 13.4050, locations) == ("Berlin", 52.5200, 13.4050)
    
    # Test from Paris
    assert flight_functions.nearest_location(
        48.8566, 2.3522, locations) == ("Paris", 48.8566, 2.3522)
    
    # Test from somewhere in the Atlantic Ocean (should return NYC as the nearest location)
    assert flight_functions.nearest_location(
        40.0, -40.0, locations) == ("NYC", 40.7128, -74.0060)
    
    # Test from somewhere in the Pacific Ocean (should return SF as the nearest location)
    assert flight_functions.nearest_location(
        37.0, -130.0, locations) == ("SF", 37.7749, -122.4194)
...
def test_find_nearest_airport():
    ...
    assert pytest.approx(
        flight_functions.find_nearest_airport(
            airport_row, 52.5200, 13.4050), 8.0, 0.1)  
    
    assert pytest.approx(
        flight_functions.find_nearest_airport(
            airport_row, 48.8566, 2.3522), 1050, 1) 
    
    assert pytest.approx(
        flight_functions.find_nearest_airport(
            airport_row, 40.7128, -74.0060), 6370, 10) 
...
def test_grab_airport_code():
    ...
    assert flight_functions.grab_airport_code(airport_data, 
    ...
    with pytest.raises(ValueError) as exc_info:
        flight_functions.grab_airport_code(invalid_data, 
                          'John F. Kennedy International Airport')
    assert str(
        exc_info.value) == "The provided DF must contain 'name' and 'iata_code' columns."
...
def test_grab_city():
    ...
    airport_data = pd.DataFrame({
        'iata_code': ['JFK', 'LAX'],
        'municipality': ['New York', 'Los Angeles']
    })
    ...
    # Test DataFrame without required columns
    invalid_data = pd.DataFrame({
        'airport_code': ['JFK', 'LAX'],
        'city': ['New York', 'Los Angeles']
    })
    with pytest.raises(ValueError) as exc_info:
        flight_functions.grab_city(invalid_data, 'JFK')
    assert str(exc_info.value) == "The provided DF must contain 'name' and 'iata_code' columns."
...
def test_format_flight_row():
    ...
    formatted_row = flight_functions.format_flight_row(
        flight_data)
...
def test_format_flights(mocker):
    ...
    mocker.patch('src.flight_functions.format_flight_row', 
                 side_effect=mock_format_flight_row)
...
def test_get_flight_offers(mocker):
    ...
    mocker.patch.object(amadeus, 'Client', 
                        autospec=True)
    mocker.patch.object(flight_functions, 'amadeus_to_dataframe', 
                        side_effect=mock_amadeus_to_dataframe)
    mocker.patch.object(flight_functions, 'format_flights', 
                        side_effect=mock_format_flights)
    mocker.patch.object(s3_functions, 'grab_data_s3', 
                        side_effect=mock_grab_data_s3)
