...
def create_hotel_dataframe(data: Dict[str, Union[List[Dict], None]]) -> pd.DataFrame:
    """
    ...
    :raises: None, falls back to reading from a constants.HOTEL_DATA_PATH
    CSV if DataFrame creation fails.
    """
    ...
    try:
        df["Score"] = (
            df["Score"].astype(str) + ": " + df["review_score_word"].astype(str)
        )
