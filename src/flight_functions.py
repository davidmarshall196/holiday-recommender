...
from src import s3_functions, constants, config
...
def nearest_location(
    lat: float, lon: float, locations: List[Tuple[str, float, float]]
    ...
    """
    return min(locations, key=lambda loc: haversine_distance(lat, lon, loc[1], loc[2]))


def find_nearest_airport(row: dict, given_lat: float, given_lon: float) -> float:
    """
    Find the Haversine distance to the nearest airport from a given
    latitude and longitude.

    :param row: A dictionary containing the name, latitude and
    longitude of an airport.
    :type row: dict
    ...
    _, lat, lon = nearest_location(
        given_lat, given_lon, [(row["name"], row["latitude_deg"], row["longitude_deg"])]
    )
            ...
            for segment in itinerary["segments"]:
                flight_detail = {
                    ...
                    "Departure Date": datetime.strptime(
                        segment["departure"]["at"], "%Y-%m-%dT%H:%M:%S"
                    ).strftime("%d/%m/%Y"),
                    "Departure Terminal": segment["departure"].get("terminal", None),
                    "Departure IATA Code": segment["departure"]["iataCode"],
                    "Arrival Date": datetime.strptime(
                        segment["arrival"]["at"], "%Y-%m-%dT%H:%M:%S"
                    ).strftime("%d/%m/%Y"),
...
def grab_airport_code(data: pd.DataFrame, airport: str) -> Union[str, None]:
    ...
    if not set(["name", "iata_code"]).issubset(data.columns):
        raise ValueError("The provided DF must contain 'name' and 'iata_code' columns.")
...
def grab_city(data: pd.DataFrame, airport_code: str) -> Union[str, None]:
    """
    ...
    cities = data[data["iata_code"] == airport_code]["municipality"].unique()
...
def format_flight_row(grouped_df):
    ...
    departure_date = datetime.strptime(
        grouped_df.iloc[0]["Departure Date"][0:10], "%d/%m/%Y"
    )
    ...
    # Total trip duration
    duration = sum(
        [parse_duration(time) for time in grouped_df["Duration"]]
    )  # in minutes
...
def format_flights(data, drop_carriers=constants.DROP_CARRIERS):
    data = data.copy()
    ...
    formatted_flights = (
        pd.DataFrame(list(formatted_flights))
        .drop_duplicates(subset=["Date", "Carriers", "Price"])
        .reset_index(drop=True)
    )
    if drop_carriers:
        formatted_flights = formatted_flights.drop("Carriers", axis=1)
...
def get_flight_offers(
    ...
    """
    Get flight offers for a given origin, destination, and month using the
    ...
    if constants.TESTING:
        ...
        df = s3_functions.grab_data_s3(constants.FLIGHTS_SAMPLE_LOC)
        return format_flights(df), airport_type

    else:
        amadeus = Client(
            client_id=config.FLIGHTS_API_KEY, client_secret=config.FLIGHTS_API_SECRET
        )
        ...
        try:
            ...
            try:
                ...
                try:
                    df = amadeus_to_dataframe(response.data, airlines)
                except Exception:
                    pass
